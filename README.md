...
This repository used AI assistants to scaffold code and draft search keywords. All relationship records, confidence scores, and source quotations were verified by the author against primary disclosures (NVIDIA SEC filings, official news, counterparty annual reports/interactive replies) before inclusion.
...
7. 当前关系数据以公开可访问的一手资料为主，部分关系只能确认到合作、部署或融资关系，不能据此推断直接采购或客户身份。
8. 部分公开披露没有给出具体交易金额、采购规模或持续时间，因此相关字段保持未知或不作推断。
9.  研究截止时间为 2026-09-18，后续公司公告、监管披露或交易变化可能导致关系状态发生变化。
10.  本项目不覆盖需要登录、付费墙、验证码、API Key 或其他受限访问条件才能获得的数据。
...
1. 更新研究截止日期。
2. 按 `docs/evidence-policy.md` 的证据规则重新检索并核验关系。
3. 更新 `data/relationships.json`。
4. 运行：
   `python scripts/validate_data.py`
5. 运行：
   `pytest -q`
...
7. 通过 `/docs` 或 README 中列出的 API 接口进行验证。

### 后续数据方向

后续可增加关系时间线、关系变更记录、供应商/客户的定量影响指标，以及更多公开监管披露和公司公告来源。
